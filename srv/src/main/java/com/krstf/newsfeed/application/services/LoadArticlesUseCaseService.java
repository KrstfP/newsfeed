...
public class LoadArticlesUseCaseService {
    ...
    private final NotifyArticleStatusChange notifyArticleStatusChange;
        ...
        for (RssFeedSource rssFeedSource : rssFeedSources) {
            ...
            for (RssItem rssItem : rssItems) {
                if (getArticle.getArticleById(rssItem.getId()).isEmpty()) {
                    ...
                    notifyArticleStatusChange.notify(new ArticleNotification(
                            rssItem.getId(),
                            NotificationObjectType.ARTICLE,
                            NotificationChangeType.ARTICLE_CREATED,
                            rssItem.getUserId(),
                            null,
                            null
                    ));
